...
void sisa_print_dump(const struct sisa_context *sisa)
{
	...
	for (i = 0; i < 8; i++) {
		printf("  r%i: 0x%04X\t\ts%i: 0x%04X\n", i, sisa->cpu.regfile.general.regs[i], i, sisa->cpu.regfile.system.regs[i]);
	}
...
void sisa_print_memory_dump(const struct sisa_context *sisa, unsigned int mem_addr)
{
	uint16_t paddr;
	uint16_t data16;
	int i;

	paddr=mem_addr-(mem_addr%2);
	for (i=0; i<12; i++) {
		data16 = sisa->memory[paddr + 1] << 8 | sisa->memory[paddr];
