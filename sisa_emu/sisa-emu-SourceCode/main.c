...
static int load_file_hex(struct sisa_context *sisa, const char *file, uint16_t addr)
{
	...
	uint16_t word;
	uint16_t offset = 0;
	const uint16_t max_size = SISA_MEMORY_SIZE - addr - 1;
		...
		if (run_mode == RUN_MODE_STEP ||
			...
			if ((c == '\t') && (run_mode == RUN_MODE_RUN)) {
				...
				if (c == 's') {
					...
					stdin_restore();
					printf("Enter memory address in hexadecimal [0000 - FFFF]: ");
					scanf ("%X",&mem_addr_dump);
					stdin_setup();
					sisa_print_memory_dump(&sisa, mem_addr_dump);
				} else if (c == 'h') {
					print_help();
				} else if (c == 'q') {
					break;
